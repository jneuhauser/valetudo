...
const WebServer = function (valetudo) {
	...
	this.app.get("/api/simple_map", function (req, res) {
		...
		if (gzippedMap) {
			self.vacuum.getCurrentStatus(function (err, data) {
				...
				let options = {gzippedMap: gzippedMap, status: data};
	...
	this.app.get("/api/get_fw_version", function (req, res) {
		...
		.then(_ => {
			...
			if (packageContent) {
				...
				valetudo = 'v' + packageContent.version + (packageContent.versionSuffix || '');
			}
		})
		.catch(_ => {})
		.finally(_ => {
	...
	this.app.post("/api/timezone", function (req, res) {
	...
	if (this.sslAvailable) {
		server.listen(this.sslPort, function(){
			console.log(new Date(), "Https server running on port " + self.sslPort)
		});
		httpServer.listen(this.port, function(){
			console.log(new Date(), "Http server running on port " + self.port + ". Redirects to https.")
		});
	} else {
		server.listen(this.port, function(){
			console.log(new Date(), "Http server running on port " + self.port)
		});
	}
};
